  ...
  class Player {
    ...
    update(input, deltaTime, enemies){
      ...
      enemies.forEach(enemy => {
        ...
        const dy = (enemy.y + this.height/2) - (this.y + this.height/2 + 20);
      ...
      if(input.keys.indexOf('ArrowRight')> -1 || input.keys.indexOf('d')> -1 ) {
        ...
        this.speed = -10;
      }else if ((input.keys.indexOf('ArrowUp')> -1 || input.keys.indexOf('swipe up')> -1 || input.keys.indexOf('w')> -1) && this.onGround()) {
        this.vy = -32;
      }else {
        this.speed = 0;
      }
  ...
  function toggleFullScreen(){
    ...
    if(!document.fullScreenElement){
      canvas.requestFullscreen().catch(err => {
        alert(`Error, can't enable fullscreen bruh: ${err.message}`);
      });
    }else{
